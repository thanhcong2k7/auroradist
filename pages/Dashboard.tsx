...
const StatCard: React.FC<{ label: string; value: string; icon: any; change?: string; loading?: boolean }> = ({ label, value, icon: Icon, change, loading }) => (
  <div className="bg-surface border border-white/5 p-5 rounded-xl hover:border-blue-500/20 transition-all group shadow-sm">
    <div className="flex justify-between items-start mb-3">
      ...
      {change && !loading && (
        <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full ${change.startsWith('+') ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
          {change}
        </span>
      )}
    </div>
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-gray-500 font-mono text-[10px] uppercase tracking-widest flex items-center gap-2 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-surface border border-white/5 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold uppercase text-[10px] tracking-widest text-blue-500">Velocity Tracking</h3>
            <span className="text-[10px] font-mono text-gray-600 uppercase border border-white/5 px-2 py-1 rounded">Last 12 Months</span>
          </div>
        ...
        <div className="bg-surface border border-white/5 rounded-2xl p-6 flex flex-col shadow-sm">
          <h3 className="font-bold uppercase text-[10px] tracking-widest text-blue-500 mb-6">Recent Activity</h3>
          ...
          <button className="mt-6 w-full py-3 bg-white/5 hover:bg-white/10 border border-white/5 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 rounded-xl">
