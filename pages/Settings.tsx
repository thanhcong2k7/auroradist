...
const Settings: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [prof, payouts] = await Promise.all([
        api.auth.getProfile(),
  ...
  const handleAddPayout = async () => {
    ...
    setIsSubmitting(true);
    const details =
      pmType === "BANK" ? `Bank Node: ...${pmAccount.slice(-4)}` : pmAccount;
  ...
  const handleDeletePayout = async (id: string) => {
    if (
      !confirm(
        "Terminate this disbursement endpoint? Authentication required for re-sync.",
      )
    )
      return;
    setIsSubmitting(true);
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-24">
        <div className="border-b border-white/5 pb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-gray-400 font-mono text-xs uppercase tracking-widest opacity-60">
              Identity & Disbursement Alignment
            </p>
          </div>
          {saveFeedback && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-surface border border-white/5 rounded-2xl overflow-hidden shadow-sm">
                <div className="p-5 border-b border-white/5 flex items-center justify-between bg-black/40">
                  <h3 className="font-bold uppercase tracking-widest text-xs text-blue-500 flex items-center gap-2">
                    <Fingerprint size={14} /> Identity Matrix
                  </h3>
                  {!isEditingProfile ? (
                    <button
                      onClick={() => setIsEditingProfile(true)}
                      className="px-4 py-2 border border-white/10 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all"
                    >
                      Modify
                    </button>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-4 py-2 text-xs font-black uppercase text-gray-400"
                      >
                        Discard
                      </button>
                      <button
                ...
                <div className="p-8 space-y-8">
                  <div className="flex items-center gap-6">
                    <div
                      ...
                      onClick={() =>
                        isEditingProfile && setShowAvatarModal(true)
                      }
                    >
                    ...
                    <div>
                      ...
                      <div
                        className={`text-[10px] font-black uppercase mt-2 inline-flex items-center gap-1 px-2 py-0.5 rounded border ${profile.role === "ADMIN" ? "border-red-500 text-red-500" : "border-blue-500/30 text-blue-500"}`}
                      >
                        {profile.role === "ADMIN" ? (
                          <Shield size={10} />
                        ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className={`w-full bg-black border rounded-xl px-4 py-3 text-xs focus:border-blue-500 outline-none transition ${isEditingProfile ? "border-blue-500/50 text-white" : "border-white/5 text-gray-400 cursor-not-allowed"}`}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-mono text-gray-400 uppercase tracking-widest ml-1">
                      ...
                      <input
                        ...
                        readOnly={!isEditingProfile}
                        className={`w-full bg-black border rounded-xl px-4 py-3 text-xs focus:border-blue-500 outline-none transition ${isEditingProfile ? "border-blue-500/50 text-white" : "border-white/5 text-gray-400 cursor-not-allowed"}`}
                        placeholder="Required for financial nodes"
                      />
                    </div>
                    <div className="md:col-span-2 space-y-1">
                      ...
                      <input
                        type="text"
                        value={
                          isEditingProfile ? tempProfile.email : profile.email
                        }
                        onChange={(e) =>
                          setTempProfile({
                            ...tempProfile,
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-surface border border-white/5 rounded-2xl overflow-hidden shadow-sm flex flex-col h-full">
                <div className="p-5 border-b border-white/5 flex items-center justify-between bg-black/40">
                  <div className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-green-500">
                    <Banknote size={16} /> Disbursement Nodes
                  </div>
                  <button
                    onClick={() => setShowPayoutModal(true)}
                    className="p-2 bg-blue-600/10 hover:bg-blue-600 text-blue-500 hover:text-white rounded-xl transition-all active:scale-90"
                  >
                    <Plus size={16} />
                  </button>
                </div>
                <div className="p-5 space-y-3 flex-1 custom-scrollbar overflow-y-auto">
                  {payoutMethods.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-white/5 rounded-xl opacity-20">
                    ...
                    payoutMethods.map((pm) => (
                      <div
                        key={pm.id}
                        className="p-4 bg-black/40 border border-white/5 rounded-xl flex items-center justify-between group hover:border-blue-500/20 transition-all"
                      >
                        <div className="flex items-center gap-4">
                          <div className="p-3 bg-white/5 rounded-lg text-gray-400 group-hover:text-blue-400 transition-colors">
                            {pm.type === "BANK" ? (
                              <CreditCard size={18} />
                            ) : (
                              <Smartphone size={18} />
                ...
                <div className="p-5 bg-blue-500/5 border-t border-blue-500/10 text-center">
                  <p className="text-[8px] text-gray-400 font-mono uppercase tracking-widest leading-relaxed">
                    Multi-DSP reconciliation feed prioritized.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-surface border border-white/10 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/40">
              <h3 className="font-bold uppercase tracking-widest text-xs text-blue-500">
                Security Override
              </h3>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="bg-yellow-500/5 p-4 rounded-xl border border-yellow-500/10 flex gap-4 text-xs text-gray-500 font-mono uppercase leading-relaxed tracking-widest">
                ...
                Action will revoke all active tokens on other devices
                immediately.
              </div>
            </div>
          ...
          <div className="bg-surface border border-white/10 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. Asset Accumulator"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-mono text-gray-500 uppercase tracking-widest ml-1">
                  ...
                  <input
                    ...
                    className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-xs focus:border-blue-500 outline-none font-mono"
                    placeholder="identity@aurora.net"
                  />
                </div>
              </div>
            </div>
            <div className="p-4 bg-black/60 border-t border-white/5 flex gap-3">
              ...
              <button
                ...
                disabled={isSubmitting || !pmName || !pmAccount}
                className="flex-1 py-3 bg-blue-600 text-white text-xs font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 active:scale-95 shadow-xl disabled:opacity-30"
              >
                {isSubmitting ? (
                  <Loader2 size={14} className="animate-spin" />
