    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="border-b border-white/10 pb-4 flex justify-between items-end">
                ...
                <button onClick={() => { setEditingDsp({ isEnabled: true }); setShowModal(true); }} className="px-4 py-2 bg-brand-primary text-white font-bold text-xs uppercase rounded-lg flex items-center gap-2">
                    <Plus size={16} /> Add DSP
                </button>
            </div>

            {loading ? <Loader2 className="animate-spin mx-auto text-brand-primary" /> : (
                    ...
                    <div className="bg-[#111] border border-white/10 rounded-xl w-full max-w-md p-6 space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="space-y-1">
                                ...
                                <input value={editingDsp.name || ''} onChange={e => setEditingDsp({ ...editingDsp, name: e.target.value })} placeholder="e.g. Spotify" className="w-full bg-black border border-white/10 rounded-lg p-3 text-xs text-white outline-none focus:border-brand-primary" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] text-gray-500 uppercase font-bold">System Code</label>
                                <input value={editingDsp.code || ''} onChange={e => setEditingDsp({ ...editingDsp, code: e.target.value.toUpperCase() })} placeholder="e.g. SPOTIFY" className="w-full bg-black border border-white/10 rounded-lg p-3 text-xs text-white outline-none focus:border-brand-primary font-mono uppercase" />
                            ...
                            <div className="space-y-1">
                                ...
                                <input value={editingDsp.logoUrl || ''} onChange={e => setEditingDsp({ ...editingDsp, logoUrl: e.target.value })} placeholder="https://..." className="w-full bg-black border border-white/10 rounded-lg p-3 text-xs text-white outline-none focus:border-brand-primary" />
                                <p className="text-[10px] text-gray-600">Only required for niche stores (e.g. ZingMP3).</p>
                            </div>
                        </div>
                        <button onClick={handleSave} className="w-full py-3 bg-brand-primary text-white font-bold text-xs uppercase rounded-lg hover:bg-brand-primary shadow-lg">Save Configuration</button>
                    </div>
                </div>
            )}
        </div>
