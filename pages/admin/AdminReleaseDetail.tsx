...
const AdminReleaseDetail: React.FC = () => {
    ...
    const loadData = async (releaseId: number) => {
        setLoading(true);
        try {
            // [UPDATE] Gọi song song: Lấy chi tiết Release VÀ Lấy danh sách DSP
    ...
    // Helper: Sort DSPs alphabetically
    const getSortedSelectedDsps = () => {
        if (!release?.selectedDsps || release.selectedDsps.length === 0 || allDsps.length === 0) return [];

        return release.selectedDsps
            .map((code: string) => {
                const found = allDsps.find(d => d.code === code);
                return found || { code, name: code, logoUrl: '' }; // Fallback nếu không tìm thấy trong list
            })
        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-[#111] p-5 rounded-xl border border-white/5 space-y-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    disabled={!isModerationMode || (!!release.upc && release.upc !== '')}
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs">
                            ...
                            <div><span className="block text-gray-500 font-mono text-[10px]">Release Date</span>{release.releaseDate}</div>
                            <div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* [MOVED & UPDATED] Selected Stores Section */}
                    <div className="bg-[#111] p-5 rounded-xl border border-white/5 space-y-4">
                        ...
                        {sortedDsps.length === 0 ? (
                            <div className="text-center py-6 text-gray-600 bg-black/20 rounded-lg">
                                <p className="text-xs font-mono">No stores selected.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                {sortedDsps.map((dsp: any) => (
                                    <div
                                        key={dsp.code}
                                        className="flex items-center gap-3 bg-black border border-white/10 p-3 rounded-lg hover:border-white/20 transition group"
                                        title={dsp.name || dsp.code}
                                    >
                                        <div className="shrink-0 bg-white/5 p-1 rounded-md group-hover:bg-white/10 transition">
                                            <DSPLogo code={dsp.code} url={dsp.logoUrl} name={dsp.name || dsp.code} size={20} />
                                        </div>
