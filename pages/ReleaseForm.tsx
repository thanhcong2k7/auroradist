...
const ReleaseForm: React.FC = () => {
  ...
  const [currentTrack, setCurrentTrack] = useState<
    Partial<Track> & { tiktokClipStartTime?: string }
  >({
    artists: [],
    contributors: [],
    hasLyrics: false,
  ...
  const [trackTab, setTrackTab] = useState<"GENERAL" | "CREDITS" | "LYRICS">(
    "GENERAL",
  );
  ...
  const [phonogramYear, setPhonogramYear] = useState(
    new Date().getFullYear().toString(),
  );
  ...
  const loadInitialData = async () => {
    setLoading(true);
    toast.loading("Loading album");
    try {
      const [fetchedLabels, fetchedDsps, fetchedTracks, fetchedArtists] =
        await Promise.all([
          api.labels.getAll(),
          api.dsps.getAll(),
          api.tracks.getAll(),
      ...
      if (release.status === "CHECKING") {
        ...
        toast.dismiss();
        toast.error("This release is locked (Processing) and cannot be edited.");
        navigate("/discography");
        return;
      }
      ...
      setCopyrightYear(
        release.copyrightYear || new Date().getFullYear().toString(),
      );
      setCopyrightLine(release.copyrightLine || "");
      setPhonogramYear(
        release.phonogramYear || new Date().getFullYear().toString(),
      );
    ...
    setLoading(false);
    toast.dismiss();
    toast.success("Album loaded successfully!");
  };
  ...
  const verifyAlbumMetadata = (): boolean => {
    ...
    if (!hasPrimaryArtist) {
      newErrors.artists = "At least one Primary Artist is required for the release.";
      isValid = false;
    }
  ...
  const validateForDistribution = (): boolean => {
    ...
    const isTikTokSelected = selectedStores.some(
      (s) => s.toLowerCase().includes("tiktok") || s === "TME",
    );
    if (isTikTokSelected) {
      const missingClip = releaseTracks.some(
        (t) => !(t as any).tiktokClipStartTime,
      );
      if (missingClip)
        errors.push("TikTok Clip Start Time (Required for all tracks)");
    }
  ...
  const validateImageDimensions = (url: string): Promise<boolean> =>
    new Promise((resolve) => {
      const img = new Image();
      img.onload = () => resolve(img.width >= 3000 && img.height >= 3000);
      img.onerror = () => resolve(false);
      img.src = url;
    });
  const handleCoverArtUpload = async (url: string) => {
    if (await validateImageDimensions(url)) {
      if (coverArt && coverArt !== url) await api.storage.delete(coverArt);
      setCoverArt(url);
    } else {
      toast.error("Image resolution too low! Minimum 3000x3000px.");
  ...
  const toggleStore = (code: string) => {
    ...
    else setSelectedStores([...selectedStores, code]);
  };
  const toggleAllStores = () => {
    const allActiveCodes = availableDsps
      .filter((d) => d.isEnabled)
      .map((d) => d.code);
    setSelectedStores(
      allActiveCodes.every((code) => selectedStores.includes(code))
        ? []
        : allActiveCodes,
    );
  };
  ...
  const handleRemoveTrack = async (trackId: number) => {
    if (
      !confirm(
        "Unlink this track from the release? It will remain in your Master Catalog.",
      )
    )
      return;
    ...
    try {
      ...
      toast.error("Failed to unlink track: ", err);
  ...
  const validateTrackForm = (): boolean => {
    ...
    const hasPrimary = t.artists?.some(
      (a) => a.role === "Primary" && a.name.trim() !== "",
    );
    if (!hasPrimary) {
    ...
    if (tkTime && tkTime.trim() !== "" && !isValidTimeFormat(tkTime)) {
      toast.error("Invalid TikTok Time format (MM:SS).");
      return false;
    }
  ...
  const handleSaveTrackAdvanced = async () => {
    ...
    try {
      ...
      if (releaseTracks.some((t) => t.id === savedTrack.id)) {
        setReleaseTracks(
          releaseTracks.map((t) => (t.id === savedTrack.id ? savedTrack : t)),
        );
      } else {
  ...
  const validateForm = (): boolean => {
    ...
    if (currentTrack.isrc && !isrcRegex.test(cleanIsrc)) {
      newErrors.isrc = "Invalid ISRC format (e.g., USLMG2400001).";
      isValid = false;
      if (!newErrors.name) setTrackTab("GENERAL");
    }

    // 3. Validate Artists
    const hasPrimary = currentTrack.artists?.some(
      (a) => a.role === "Primary" && a.name.trim() !== "",
    );
    if (!hasPrimary) {
      newErrors.artists = "At least one Primary Artist is required.";
    ...
    const hasComposer = currentTrack.contributors?.some(
      (c) => c.role === "Composer" && c.name.trim() !== "",
    );
    const hasProducer = currentTrack.contributors?.some(
      (c) => c.role === "Producer" && c.name.trim() !== "",
    );
    const hasLyricist = currentTrack.contributors?.some(
  ...
  const updateArtist = (i: number, f: keyof TrackArtist, v: any) => {
    const a = [...(currentTrack.artists || [])];
    a[i] = { ...a[i], [f]: v };
    setCurrentTrack({ ...currentTrack, artists: a });
  };
      ...
      <div className="sticky top-0 z-30 bg-[#0A0A0A] border-b border-white/10 px-6 lg:px-8 pt-6 lg:pt-8 pb-4 backdrop-blur-md bg-opacity-90">
        <div className="max-w-6xl mx-auto w-full flex justify-between items-end">
          <div>
            <div className="flex items-center gap-3 text-gray-500 font-mono text-xs mb-2">
              ...
              <span>//</span>
              <span
                className={`px-2 py-0.5 rounded border ${status === "CHECKING" ? "border-yellow-500/30 text-yellow-400 bg-yellow-500/10" : "border-gray-500/30 text-gray-400 bg-gray-500/10"}`}
              >
                {status}
              </span>
            </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-3 py-2 border border-white/10 hover:bg-white hover:text-black text-white font-bold uppercase rounded transition flex items-center gap-2 text-sm"
            >
              <Eye size={14} />
            </button>
            <button
              ...
              disabled={loading}
              className="px-4 py-2 border border-white/10 hover:bg-white hover:text-black text-white font-bold uppercase rounded transition flex items-center gap-2 text-sm disabled:opacity-50"
            >
              <Save size={14} /> Save Draft
            </button>
            <button
              onClick={() => handleSave("CHECKING")}
              disabled={loading}
              className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase rounded shadow-[0_0_20px_rgba(37,99,235,0.3)] transition flex items-center gap-2 text-sm disabled:opacity-50"
            >
              {loading ? (
            ...
            <button
              ...
              className="text-gray-500 hover:text-white transition px-2"
            >
              <X size={24} />
            </button>
          </div>
        ...
        {validationErrors.length > 0 && (
          <div className="max-w-6xl mx-auto w-full mt-4 bg-red-500/10 border border-red-500/20 p-3 rounded-lg flex gap-3 items-center animate-pulse">
            ...
            <span className="text-red-400 text-xs font-mono uppercase font-bold">
              Action Required: {validationErrors.join(", ")}
            </span>
          </div>
        )}
      </div>
          ...
          <div className="flex border-b border-white/10 mt-6">
            {[1, 2, 3].map((step) => (
              <button
                ...
                className={`px-6 py-3 font-bold text-sm uppercase tracking-wider transition-colors flex items-center gap-2 border-b-2 ${currentStep === step ? "text-blue-500 border-blue-500" : "text-gray-400 hover:text-gray-400 border-transparent"}`}
              >
                {step === 1
                  ? "1. Metadata"
                  : step === 2
                ...
                <div className="bg-surface border border-white/5 rounded-xl overflow-hidden">
                  <div className="p-6 border-b border-white/10 flex justify-between items-center">
                    <h3 className="font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                      <Disc size={16} /> Tracklist{" "}
                      <span className="text-red-500">*</span>
                    </h3>
                    <button
                      onClick={openTrackManager}
                      className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase rounded text-xs flex items-center gap-2 transition"
                    >
                      ...
                      {releaseTracks.map((track, idx) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 font-mono text-xs text-blue-400">
                            {(track as any).tiktokClipStartTime || "00:00"}
                          </td>
                ...
                <div className="bg-surface border border-white/5 rounded-xl p-6">
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    <div
                      ...
                      className={`flex-1 p-4 rounded-xl border cursor-pointer transition ${isWorldwide ? "bg-blue-600/10 border-blue-500 text-white" : "bg-black/40 border-white/10 text-gray-500"}`}
                    >
                        ...
                        .map((dsp) => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-4 rounded-xl border cursor-pointer transition flex items-center gap-3 ${isSelected ? "bg-blue-600/10 border-blue-500/50" : "bg-black border-white/10 text-gray-500"}`}
                            >
          ...
          <div className="w-1/3 flex justify-start">
            {currentStep > 1 && (
              ...
              >
                ...
                <span className="text-xs font-bold uppercase text-gray-300 group-hover:text-white transition-colors tracking-widest">
                  Back
                </span>
              </button>
            )}
          ...
          <div className="bg-surface border border-white/10 rounded-xl w-full max-w-3xl h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            {modalView === "EDIT" && Object.keys(trackErrors).length > 0 && (
              <div className="bg-red-500/10 border-b border-red-500/20 px-6 py-2 flex items-center gap-2">
                ...
                <span className="text-xs text-red-400 font-mono font-bold">
                  Please fix errors in highlighted tabs.
                </span>
              </div>
            )}
            ...
            {modalView === "EDIT" && (
              <div className="flex border-b border-white/5 bg-black/60">
                <button
                  ...
                  className={`flex-1 py-3 text-xs font-black uppercase tracking-widest border-b-2 transition ${trackTab === "GENERAL" ? "border-blue-500 text-blue-400 bg-blue-500/5" : "border-transparent text-gray-400"} ${trackErrors.name ? "text-red-400" : ""}`}
                >
                  1. General
                </button>
                <button
                  onClick={() => setTrackTab("CREDITS")}
                  className={`flex-1 py-3 text-xs font-black uppercase tracking-widest border-b-2 transition ${trackTab === "CREDITS" ? "border-blue-500 text-blue-400 bg-blue-500/5" : "border-transparent text-gray-400"} ${trackErrors.artists || trackErrors.contributors ? "text-red-400" : ""}`}
                >
                  2. Credits
                </button>
                <button
                  onClick={() => setTrackTab("LYRICS")}
                  className={`flex-1 py-3 text-xs font-black uppercase tracking-widest border-b-2 transition ${trackTab === "LYRICS" ? "border-blue-500 text-blue-400 bg-blue-500/5" : "border-transparent text-gray-400"}`}
                >
                  3. Content
                </button>
              </div>
                  ...
                  {availableTracks.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64 text-gray-500 gap-2">
                      ...
                      <p className="text-xs font-mono">
                        No tracks in vault. Add a new one below.
                      </p>
                    </div>
                  ) : (
                    availableTracks.map((track) => {
                      const isAdded = releaseTracks.some(
                        (t) => t.id === track.id,
                      );
                      return (
                        <div
                          key={track.id}
                          onClick={() =>
                            !isAdded && handleAddTrackToRelease(track)
                          }
                          className={`p-3 flex items-center justify-between border border-transparent rounded-lg hover:bg-white/5 transition cursor-pointer group ${isAdded ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                      ...
                      <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between">
                        <div>
                          <p className="text-sm font-bold text-white flex items-center gap-2">
                            <Clock size={16} /> TikTok Clip Start
                          </p>
                          <p className="text-xs text-gray-400 mt-1">
                            Defaults to 00:00 if empty.
                          </p>
                        </div>
                        <input
                          type="text"
                          ...
                          onChange={(e) =>
                            setCurrentTrack({
                              ...currentTrack,
                              tiktokClipStartTime: e.target.value,
                            })
                          }
                          placeholder="00:00"
                          ...
                          {currentTrack.artists?.map((a, i) => (
                            <div key={i} className="flex gap-2">
                              <select
                                value={a.role}
                                onChange={(e) =>
                                  updateArtist(i, "role", e.target.value)
                                }
                                className="w-24 bg-black border border-white/10 rounded px-2 py-2 text-xs outline-none"
                              >
                                <option value="Primary">Primary</option>
                          ...
                          {currentTrack.contributors?.map((c, i) => (
                            <div key={i} className="flex gap-2">
                              ...
                              <input
                                ...
                                className="flex-1 bg-black border border-white/10 rounded px-3 py-2 text-sm outline-none"
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-between bg-black/40 p-4 rounded-xl border border-white/5">
                        <p className="text-sm font-bold">
                          Does this track have lyrics?
                        </p>
                        <div className="flex gap-4">
                          <label className="flex items-center gap-2 cursor-pointer text-xs">
                            <input
                              type="radio"
                              checked={!currentTrack.hasLyrics}
                              onChange={() =>
                                setCurrentTrack({
                                  ...currentTrack,
                                  hasLyrics: false,
                        ...
                        <div className="space-y-4 pl-4 border-l-2 border-white/10">
                          ...
                          <div className="p-4 rounded-xl border border-white/10 space-y-4">
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="font-bold text-sm text-wider uppercase">
                                  Explicit Content
                                </p>
                                <p className="text-xs text-gray-400 text-wider font-mono">
                                  Sensitive vocabulary detected?
                                </p>
                              </div>
                              <input
                                type="checkbox"
                                checked={currentTrack.isExplicit}
                                onChange={(e) =>
                                  setCurrentTrack({
                                    ...currentTrack,
            ...
            <div className="p-4 border-t border-white/10 bg-black/40 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 border border-white/10 rounded text-xs text-white hover:bg-white/5 transition"
              >
                Close
              </button>
              {modalView === "BROWSE" && (
              ...
              {modalView === "EDIT" && (
                <button
                  onClick={() => handleSaveTrackAdvanced()}
                  className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase rounded text-xs shadow-lg flex items-center gap-2"
                >
