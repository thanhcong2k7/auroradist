...
BEGIN
  ...
  SELECT COALESCE(SUM(streams), 0)::TEXT INTO total_streams FROM public.analytics_monthly;
  SELECT COALESCE(available_balance, 0)::TEXT INTO total_revenue FROM public.wallet_summary LIMIT 1;
  SELECT COUNT(*)::TEXT INTO active_releases FROM public.releases WHERE status = 'ACCEPTED';

  RETURN json_build_object(
    'totalStreams', total_streams,
    'revenue', total_revenue,
    'activeReleases', active_releases,
    'totalStreamsChange', '+12.5%', -- Mock data for UI
    'revenueChange', '+8.2%',       -- Mock data for UI
    'monthlyListeners', '85,420',   -- Mock data for UI
    'monthlyListenersChange', '+24%' -- Mock data for UI
  );
END;
$$;
...
BEGIN
  ...
  INSERT INTO public.wallet_summary (user_id)
  VALUES (new.id);
...
CREATE POLICY "Enable read access for authenticated users" ON "public"."tracks" FOR SELECT TO "authenticated" USING (true);
